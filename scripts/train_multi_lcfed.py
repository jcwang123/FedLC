...
parser.add_argument('--fed', type=int, default=1, help='using fed or not')
    ...
    for epoch_num in range(max_epoch):
        ...
        for client_idx in range(args.client_num):
            ...
            for i_batch, sampled_batch in enumerate(dataloader_current):
                ...
                pred, hmap = net_current(volume_batch, client_idx, 0, None)

                c_loss = 0
                for other_client in range(args.client_num):
                    with torch.no_grad():
                        _, _hmap = net_current(volume_batch, other_client, 0,
                                               None)
                    if not other_client == client_idx:
                        c_loss += c_loss_func(hmap[-1], _hmap[-1])
                        c_loss = c_loss / len(hmap)
                c_loss = -c_loss / (args.client_num - 1)

                s1_loss = dice_loss(pred, label_batch)

                loss = s1_loss + c_loss * args.alpha
        ...
        for site_index in range(args.client_num):
            ...
            score_values = test_lc_unet(site_index, this_net, None, args)
